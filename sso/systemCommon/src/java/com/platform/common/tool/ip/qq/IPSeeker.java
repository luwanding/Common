...
public class IPSeeker {
	...
	public IPSeeker(String fileName, String dir) throws Exception {
		...
		try {
			...
			ipFile = new RandomAccessFile(dir + IP_FILE, "r");
	 ...
	 * @return 国家名字符串
	 */
	public String getCountry(byte[] ip) throws Exception {
		// 检查ip地址文件是否正常
		if (ipFile == null) {
	...
	public String getCountry(String ip) throws Exception {
		...
		if (ip != null && !ip.trim().equals("")) {
			...
			String country = getCountry(Util.getIpByteArrayFromString(ip));
	 ...
	 * @return IPLocation结构
	 */
	private IPLocation getIPLocation(byte[] ip) throws Exception {
	 ...
	 * @param ip
	 */
	private void readIP(long offset, byte[] ip) throws Exception {
	 ...
	 */
	private long locateIP(byte[] ip) throws Exception {
	 ...
	 * @return IPLocation对象
	 */
	private IPLocation getIPLocation(long offset) throws Exception {
		try {
			// 跳过4字节ip
	 ...
	 * @return IPLocation对象
	 */
	private IPLocation getIPLocation(int offset) throws Exception {
		// 跳过4字节ip
	 ...
	 * @return 地区名字符串
	 */
	private String readArea(int offset) throws Exception {
	 ...
	 * @return 读取的字符串，出错返回空字符串
	 */
	private String readString(int offset) throws Exception {
