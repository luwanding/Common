 ...
 * 
 */
public final class SecurityManagerPKCS5Padding {
	private final String desStr = "12345678";
	private final String algorithm = "DESede/ECB/PKCS5Padding";

	private static SecurityManagerPKCS5Padding instance;

	private final byte[] desInitValue = HexUtils.strToByte(desStr);// 默认向量
	

	/**
	 * 屏蔽构造方法
	 */
	private SecurityManagerPKCS5Padding() {
	 ...
	 * @return
	 */
	static SecurityManagerPKCS5Padding getInstance() {
		if (instance == null) {
			synchronized (SecurityManagerPKCS5Padding.class) {
				if (instance == null) {
					//Security.addProvider(new com.sun.crypto.provider.SunJCE());
					//Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());// 添加PKCS7Padding支持
					instance = new SecurityManagerPKCS5Padding();
				}
			}
		}
		return instance;
